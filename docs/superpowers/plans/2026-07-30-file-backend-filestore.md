...
- **Mutation-check every guard clause.** After a guard passes, reintroduce the specific defect it guards and confirm the test fails. Each *clause* of a multi-clause guard (`validate_bucket_name`, `_needs_overflow`, containment, collision) gets its own reintroduction, not a representative subset. A test that passes against the reintroduced bug is testing nothing. **Defense-in-depth carve-out:** a clause that is provably subsumed by a stricter downstream clause (an *equivalent mutant* — e.g. `validate_bucket_name`'s explicit `..`/slash/NUL/length checks under the stricter `_BUCKET_RE`; `constrained_rmtree`'s `realpath` parent-check given a pre-canonicalized root plus the `islink` guard) is exempt from individual-killability, PROVIDED (a) the load-bearing clause it defers to *is* mutation-killed by a test, and (b) the redundant clause is documented in code as intentional defense-in-depth. Deleting a security backstop merely to make a metric go green is the wrong trade; documenting the equivalence honestly is the right one.
...
`validate_bucket_name` — only the LOAD-BEARING clauses are individually killable; verify each:
- char-class regex (`_BUCKET_RE`) → delete it and `"Upper"` is wrongly accepted → `test_rejects_traversal_and_illegal` FAILS.
- goog-reserved clause → delete it and `"goog-x"` is wrongly accepted → the test FAILS.

The explicit NUL / leading-slash / `..`-segment / traversal-loop / slash / length clauses are **intentional defense-in-depth and equivalent mutants** — `_BUCKET_RE` (`^[a-z0-9][a-z0-9._\-]{1,61}[a-z0-9]$`) already rejects every input they catch (any `/`, uppercase, out-of-range length via `{1,61}`+anchors, NUL, leading/trailing dot), so deleting one does NOT fail a test. That is expected under the Global-Constraints defense-in-depth carve-out; the function's docstring records it. Do NOT weaken `_BUCKET_RE` to make them killable, and do NOT delete them — they are the backstop if the regex is ever relaxed and they give precise errors.
...
- `constrained_rmtree` — its THREE load-bearing guards are each killable via `test_constrained_rmtree_refuses_symlink_non_child_unindexed`: (1) remove the `islink`/`isdir` real-directory guard → the direct-symlink (`root/evil`) sub-case is wrongly allowed → FAILS; (2) remove the direct-child parent check → the sibling-real-dir (`outside`) sub-case FAILS; (3) remove the `index_names` clause → the unindexed (`bucket-y`) sub-case FAILS. The `realpath` (vs string `normpath`) refinement on the parent check is **defense-in-depth and an equivalent mutant** — because `FileStore.__init__` pre-canonicalizes its root and the `islink` guard already rejects a symlinked final component, no reachable call makes `realpath` and `normpath` disagree, so the normpath mutation SURVIVES by design (documented in the function). Keep `realpath`; it is the backstop for a future non-canonical-root caller. Also: remove `assert_posix_support` from the module and confirm `test_posix_support_asserted` FAILS on import/attribute.
