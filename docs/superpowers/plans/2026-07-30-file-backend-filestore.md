...
- **Zero new runtime dependencies.** `setup.py` stays as-is. The file backend uses only stdlib + already-present `crc32c`/`protobuf`. `hypothesis`/`coverage` are dev deps in `flake.nix` only.
